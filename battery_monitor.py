import psutil
import smtplib
from email.mime.text import MIMEText

# Function to check battery level
def check_battery():
    battery = psutil.sensors_battery()
    percent = battery.percent
    return percent

# Function to send email alert
def send_email():
    sender_email = "suyogpatil58@gmail.com"
    receiver_email = "suyogpatil58@gmail.com"
    aws_smtp_server = "email-smtp.us-east-1.amazonaws.com"
    aws_smtp_port = 587
    aws_smtp_username = "AKIAXTORPHBYV5R2PIPV"
    aws_smtp_password = "BN4XI+eG6sFJ4kRWBiWbLQStUTBE6tlIsk5LxWGRGTzx"

    subject = "Laptop Battery Low Warning!"
    message = f"Warning: Your laptop battery is at {check_battery()}%. Please charge it soon!"

    msg = MIMEText(message)
    msg["Subject"] = subject
    msg["From"] = sender_email
    msg["To"] = receiver_email

    try:
        server = smtplib.SMTP(aws_smtp_server, aws_smtp_port)
        server.starttls()
        server.login(AKIAXTORPHBYV5R2PIPV, BN4XI+eG6sFJ4kRWBiWbLQStUTBE6tlIsk5LxWGRGTzx)
        server.sendmail(suyogpatil58@gmail.com, suyogpatil58@gmail.com, msg.as_string())
        server.quit()
        print("Email alert sent successfully!")
    except Exception as e:
        print(f"Error sending email: {e}")

# Run monitoring
battery_percentage = check_battery()
if battery_percentage <= 30:
    send_email()
else:
    print(f"Battery Level: {battery_percentage}%, no action needed.")
